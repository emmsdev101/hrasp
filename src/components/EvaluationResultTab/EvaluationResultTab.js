...
export default function EvaluationResultTab({ panel, head, committee }) {
  ...
  return viewDetails ? (
    ...
    <Result
      ...
      viewEvaluationResult={viewEvaluationResult}
    />
  );
}
...
const Result = ({
  ...
  const teaching = ["COE", "COA", "CBM", "SICT"];
  ...
  const fetchEvaluations = async () => {
    ...
    let reqUrl =
      apiBaseUrl +
      `/admin/getEvaluationResults/${type}/${selectedDepartment}/${selectedPosition}`;
    if (head)
  ...
  const fetchPositions = async () => {
    const getPositions = await axios.get(
      apiBaseUrl + `/admin/getJobPositions/${type}/${selectedDepartment}`,
      { withCredentials: true }
    );
        ...
        <Card>
          <Card.Header className="p-3 m-3">
            <Row className="d-flex justify-content-between align-items-center">
              <Col className="col-sm-12 mb-2 col-md-3 ">
          ...
          <Card.Body className="p-3">
            {renderTable ? (
              <EvaluationTable
                tableRef={tableRef}
                action={panel ? viewEvaluation : viewEvaluationResult}
                data={evaluations}
                panel={panel}
              />
            ) : (
      ...
      <Col>
        <Card>
          <Card.Header className="p-3 m-3">
            <Container
              fluid
              className="mb-2 p-0 d-flex justify-content-start alert-items-center"
            >
              <FontAwesomeIcon
                icon={faArrowLeft}
                size="lg"
                onClick={() =>
                  (window.location.href = "../applications/evaluation-result")
                }
              />

              <p className="p-2 m-0 me-2 alert alert-info">
                Applying: {data.title}
              </p>
              <p className="p-2 m-0 me-2 alert alert-info">
                For: {data.department}
              </p>
              <p className="alert alert-danger m-0 p-2 ms-auto w-auto">
                Total: {data.total}
              </p>
            </Container>
            <Row>
              ...
              <Col className="d-flex justify-content-end align-items-center">
                ...
                />
                 <DownloadTableExcel
                  filename={data.applicant_name + "_evaluation"}
                  sheet="users"
                  currentTableRef={tableRef.current}
                >
                  <Button variant="light" size="sm">
